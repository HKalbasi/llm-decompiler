...
#figure(
  ...
  caption: [
    نتیجه آزمون مدل‌های زبانی با کدی که در آن عدد جادویی
    تغییر یافته است
  ],
)
...
#figure(
  ...
  caption: [
    نتیجه آزمون مدل‌های زبانی با کدی که در آن با تکنیک تزریق درخواست
    یک تابع مخرب مخفی شده است
  ],
)

اگرچه ممکن است با پیشرفت مدل‌ها یا تکنیک‌های پرامپت دهی پیشرفته تر مانع این نوع
حملات شد، و تلاش‌های خوبی در این زمینه صورت گرفته است
@DefendLLM
اما از طرف مقابل مهاجمین نیز می‌توانند تکنیک‌های پیشرفته‌تری را به کار
بگیرند و در این زمینه نیز تحقیقاتی انجام شده است @AttackLLM @BypassDefendLLM.
در نتیجه تا وقتی که یک تضمین ریاضیاتی و قطعی روی خروجی مدل‌ها وجود نداشته
...
دستور زبان را صفر در نظر بگیریم @Grammar_LLM.
...
#figure(rotate(180deg, image("../seminar/maze2.png", width: 50%)), caption: [
  انتخاب یک مسیر به هدف با رعایت قوانین و محدودیت‌ها به صورت
