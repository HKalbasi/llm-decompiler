...
مبدل نمایش میانی به زبان ماشین نوشته شود @lattner2004llvm.
...
استفاده مشترک کرد @lattner2004llvm.
...
با یک الگوریتم ساده گراف قابل تصمیم‌گیری است @CFG.
...
#notes.note[غیر درون‌خط‌سازی][de-inlining]
توابع روی این نمایش میانی انجام شود، سپس کد سطح بالا از روی نمایش میانی
...
دستورالعمل‌های بلوک پایه از دستورهای پیچیده پشتیبانی کردیم تا عملیات‌های
...
== نمای کلی پیاده‌سازی لایه‌ای مترجم معکوس

در تصویر زیر اجزای اصلی مترجم معکوس پیشنهاد شده
و توالی اجرا شدن آن‌ها معین شده است.

#figure(
  image("../images/flow.drawio.png"),
  caption: [
    نمای کلی از پیاده‌سازی لایه‌ای مترجم معکوس
  ],
)

اجزایی که با رنگ آبی مشخص شده‌اند توسط مدل زبانی بزرگ
اجرا می‌شوند و اجزای سبز رنگ پیاده‌سازی قطعی و
الگوریتمی دارند و نتایج تولید شده توسط مدل زبانی
را نیز صحت سنجی می‌کنند.
...
#heading(level: 3, outlined: false)[گیدرا]
...
کد خروجی سی ساخته می‌شود و متن سی از روی آن درخت نحوه تولید می‌شود @ghidra.
بنابراین
می‌توان گفت که گیدرا در مجموع به نوعی از معماری لایه‌ای پیروی می‌کند.

#heading(level: 3, outlined: false)[رت‌دک]
...
ساخته شده برای مترجم ال‌ال‌وی‌ام استفاده کند @RetDec.

#heading(level: 3, outlined: false)[سالت برای ترجمه معکوس]
...
کنترل می‌سازد و آن را به مدل زبانی ورودی می‌دهد @SALT4Decompile.
...
#figure(
  image("../images/go_back_and_forth.drawio.png", width: 80%),
  caption: [
