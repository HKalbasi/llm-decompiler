...
#notes.note[بلوک پایه][basic block]
وجود دارد. این بلوک‌های پایه شامل تعدادی
...
راس‌های آن بلوک‌های پایه و یال‌های آن خاتمه‌دهنده‌ها هستند. یال‌های این
...
یک بلوک پایه تعدادی دستور و یک خاتمه دهنده وجود دارد. دستورها توانایی
...
اجازه پرش به ابتدای بلوک‌های پایه دیگر را دارند.

این طراحی بلوک‌های پایه به بسیاری از تحلیل‌های روی برنامه کمک می‌کند تا از پیچیدگی
حاصل از پرش‌ها جلوگیری شود، به دلیل این که درون یک بلوک پایه هیچ پرشی وجود
ندارد و هیچ پرشی به وسط آن نیز انجام نمی‌شود بنابراین ما می‌توانیم در نظر بگیریم
که همه دستورهای یک بلوک پایه همواره اجرا می‌شوند. به دلیل این کمک، مترجم‌ها
...
بر خلاف تعریف بلوک پایه که یک تعریف استاندارد و متداول در گراف‌های جریان کنترل
...
متغیر محلی نوع دار و تعدادی بلوک پایه نمایش داده می‌شود. برای ساخت
...
به طور مستقیم به مدل زبانی داده می‌شوند و از مدل زبانی خواسته می‌شود
تا امضای تابع را تشخیص دهد. در صورت برابری کامل امضای تابع با
امضای واقعی، پاسخ مدل درست در نظر گرفته می‌شود.

#figure(
  table(
    columns: 2,
    table.header("نام مدل", "درصد خروجی‌های درست"),

    "چت جی‌پی‌تی ۵", "۹۵",
    "دیپ‌سیک با تفکر", "۹۵",
    "دیپ‌سیک", "۸۰",
    "کوئن چهار میلیارد پارامتری", "۶۰",

    table.cell(colspan: 2)[پژوهش‌های پیشین],
    "گیدرا", "۵۵",
  ),
  caption: [
    آزمایش توانایی مدل‌های زبانی در یافتن امضای توابع
  ],
)

این آزمایش نشان می‌دهد که مدل‌های زبانی در این وظیفه توان بسیار بالایی
دارند و چالش خاصی در این وظیفه برای آن‌ها وجود ندارد. با این حال این کار
برای روش‌های بدون مدل زبانی کار بسیار دشوارتر است. مثلا گیدرا حتی از
ضعیف‌ترین مدل زبانی آزمایش شده نیز امتیاز پایین‌تری گرفته است، چون
بسیار محافظه‌کار عمل می‌کند و در مواقعی که ورودی یا خروجی به صورت
قطعی معلوم نیست، از نوع تعریف نشده برای اشاره به آن‌ها استفاده می‌کند و
امتیاز این مرحله را از دست می‌دهد. اما مدل زبانی با استفاده از درک خود
از طرز کارکرد تابع می‌تواند حدس خودش را جایگزین یک خروجی محافظه‌کارانه
و غیرقابل استفاده کند. متاسفانه پژوهش‌های پیشین مبتنی بر
مدل‌های زبانی به صورت لایه‌ای کار نمی‌کنند و به صورت سرتاسر پیاده‌سازی
شده‌اند و در نتیجه به صورت مستقیم در این زمینه قابل مقایسه نیستند و در
این لایه برای سنجش فقط به مدل قطعی گیدرا بسنده کردیم.

== ساخت نمایش میانی از روی کد زبان ماشین

با دانستن نوع‌های ورودی‌ها و خروجی تابع، ساخت نمایش میانی از روی کد ماشین راه‌حل الگوریتمی دارد. این کار عملا معادل ساخت یک گراف جریان کنترل
از روی کد زبان ماشین است که در پژوهش‌های پیشین نیز انجام شده است @SALT4Decompile.

برای این کار کافیست که دستورهای زبان ماشین به دو دسته پرش کننده و بدون
پرش تقسیم شوند. هدف‌های پرش در دستورات پرشی بلوک‌های پایه جدید می‌سازند و
خود دستورات پرشی خاتمه‌دهنده‌های بلوک‌های پایه خواهند بود. دستورهای غیر
پرشی نیز دستورهای بلوک پایه را تشکیل می‌دهند و صرفا باید به دستورهای تعریف‌شده
در نمایش میانی ما تبدیل شوند.

== بهبود نمایش میانی

کد ساخته شده در نمایش میانی هم‌چنان بسیار مشابه کد ماشین ورودی است و
فقط به جای دستورات ماشین از دستورات نمایش میانی استفاده می‌کند. در
این قسمت با استفاده از تعدادی عملیات الگوریتمی و عملیات‌هایی برای
مدل زبانی تلاش می‌کنیم تا در همین نمایش میانی کد سطح بالاتر و خواناتری
تولید کنیم. با خوانا سازی کد در لایه نمایش میانی دیگر هنگام تبدیل
کد به زبان برنامه‌نویسی مقصد یا بر روی خروجی زبان مقصد نیاز به خواناسازی
کمتری خواهیم داشت.
...
- برخط سازی مقداردهی متغیر: در صورتی که مقدار یک متغیر محلی در بلوک‌های پایه
...
معادل بودن آن‌ها تصمیم پذیر نیست، درون یک بلوک پایه به علت عدم وجود ساختار‌های
...
به نام
#notes.note[زد۳][Z3]
استفاده کردیم. زد۳ توسط تیم تحقیقاتی مایکروسافت توسعه داده شده است
...
پشتیبانی می‌کند که برای کاربرد بررسی ما بسیار مفید است @z3.
از زد۳ در پروژه‌های بزرگ
صحت‌سنجی رسمی مثل دفنی، بوگی، پراستی و ... استفاده شده است.

=== بررسی عملیات‌های مدل زبانی با زد۳

برای صحت‌سنجی عملیات‌هایی که مدل زبانی روی مجموعه دستورهای یک بلوک پایه می‌تواند
